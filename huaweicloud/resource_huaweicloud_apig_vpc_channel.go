...
func buildApigVpcChannelMembers(d *schema.ResourceData, client *golangsdk.ServiceClient) ([]channels.MemberInfo, error) {
	...
	for i, v := range members {
		...
		if id, ok := member["id"]; memberType == "ecs" && ok {
			...
			server, err := cloudservers.Get(client, id.(string)).Extract()
			if err != nil {
				return result, fmtp.Errorf("Unable ot find ecs")
			}
...
func buildApigVpcChannelParameters(d *schema.ResourceData, client *golangsdk.ServiceClient) (channels.ChannelOpts, error) {
	...
	// Backend servers
	members, err := buildApigVpcChannelMembers(d, client)
	if err != nil {
		return opt, err
	}
	opt.Members = members
...
func resourceApigVpcChannelV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmtp.Errorf("Error creating HuaweiCloud ECS v1 client: %s", err)
	}
	opts, err := buildApigVpcChannelParameters(d, ecsClient)
	if err != nil {
...
func resourceApigVpcChannelV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	opt, err := buildApigVpcChannelParameters(d, ecsClient)
	if err != nil {

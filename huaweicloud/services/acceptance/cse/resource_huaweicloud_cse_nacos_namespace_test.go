...
func getNacosNamespaceFunc(cfg *config.Config, state *terraform.ResourceState) (interface{}, error) {
	...
	return cse.QueryNacosNamespace(client, state.Primary.Attributes["engine_id"], state.Primary.ID)
}
...
func TestAccNacosNamespace_basic(t *testing.T) {
	var (
		...
		rc           = acceptance.InitResourceCheck(resourceName, &service, getNacosNamespaceFunc)
	)
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccNacosNamespace_basic(name),
			...
			{
				Config: testAccNacosNamespace_basic(updateName),
				Check: resource.ComposeTestCheckFunc(
					rc.CheckResourceExists(),
...
func testAccNacosNamespace_basic(name string) string {
	return fmt.Sprintf(`

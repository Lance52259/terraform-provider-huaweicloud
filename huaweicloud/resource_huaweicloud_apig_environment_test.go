...
func testAccCheckApigEnvironmentDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := getEnvironmentFormServer(client, rs.Primary.Attributes["instance_id"], rs.Primary.ID)
		if err == nil {
...
func testAccCheckApigEnvironmentExists(name string, env *environments.Environment) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		found, err := getEnvironmentFormServer(client, rs.Primary.Attributes["instance_id"], rs.Primary.ID)
		if err != nil {

...
func createApigGroupCustomResponses(client *golangsdk.ServiceClient, instanceId, groupId string,
	...
	for _, response := range resps.List() {
		...
		_, err := responses.Create(client, opt).Extract()
		if err != nil {
			return err
		}
	}
...
func removeApigGroupCustomResponses(client *golangsdk.ServiceClient, instanceId, groupId string,
	resps *schema.Set) error {
	for _, response := range resps.List() {
		respMap := response.(map[string]interface{})
...
func resourceApigGroupV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if environments, ok := d.GetOk("environments"); ok {
		...
		if err != nil {
			return fmt.Errorf("Binding environment variables failed: %s", err)
		}
	}
	if customResps, ok := d.GetOk("custom_responses"); ok {
		...
		if err != nil {
			return fmt.Errorf("Creating custom responses failed: %s", err)
		}
	}
...
func getCustomResponsesById(client *golangsdk.ServiceClient, instanceId, groupId,
	id string) ([]map[string]interface{}, error) {
	groupResps, err := responses.Get(client, instanceId, groupId, id).Extract()
	if err != nil {
		return make([]map[string]interface{}, 0), fmt.Errorf("Error getting response rule from specifies gorup (%s)"+
			"by response id (%s)", groupId, id)
	}
	respRules := make([]map[string]interface{}, 0, len(groupResps.Responses))
	for errorType, rule := range groupResps.Responses {
		if rule.IsDefault {
	...
	return respRules, nil
}

// customResps
func setApigGroupCustomResponses(d *schema.ResourceData, client *golangsdk.ServiceClient,
	groupResps []responses.Response) error {
	instanceId := d.Get("instance_id").(string)
	result := make([]map[string]interface{}, 0, len(groupResps))
	for _, groupResp := range groupResps {
		if groupResp.IsDefault {
			continue
		}
		respRules, err := getCustomResponsesById(client, instanceId, d.Id(), groupResp.Id)
		if err != nil {
			return err
		}
		if len(respRules) == 0 {
...
func getApigGroupEnvironmentVariables(d *schema.ResourceData,
	...
	if err != nil {
		return []environments.Variable{}, fmt.Errorf("Error getting environment variable list from server: %s", err)
	}
	result, err := environments.ExtractVariables(pages)
	if err != nil {
		return []environments.Variable{}, fmt.Errorf("Error extract variables: %s", err)
	}
	return result, nil
}
...
func getApigGroupCustomResponses(d *schema.ResourceData,
	...
	if err != nil {
		return []responses.Response{}, fmt.Errorf("Error getting custom responses list from server: %s", err)
	}
	result, err := responses.ExtractResponses(pages)
	if err != nil {
		return []responses.Response{}, fmt.Errorf("Error extract custom responses: %s", err)
	}
	return result, nil
}
...
func resourceApigGroupV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error getting APIG v2 group: %s", err)
	}
	if err = setApigGroupParamters(d, config, resp); err != nil {
		return fmt.Errorf("Error saving group to state: %s", err)
	}
	...
	if err = setApigGroupEnvironmentVariables(d, variables); err != nil {
		return fmt.Errorf("Error saving variables to state: %s", err)
	}
	...
	if err = setApigGroupCustomResponses(d, client, responses); err != nil {
		return fmt.Errorf("Error saving environments to state: %s", err)
	}
	return nil
}
...
func updateApigResponsesCustomResps(d *schema.ResourceData, client *golangsdk.ServiceClient) error {
	...
	err := removeApigGroupCustomResponses(client, instanceId, d.Id(), removeRaws)
	if err != nil {
		return fmt.Errorf("Removing custom responses failed: %s", err)
	}
...
func resourceApigGroupV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("environments") {
		if err := updateApigGroupEnvironmentVariables(d, client); err != nil {
			return fmt.Errorf("Error updating HuaweiCloud APIG environment variables for the group (%s): %s", d.Id(), err)
		}
	}

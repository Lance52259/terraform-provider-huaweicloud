...
func createApigV2ApplicationCode(d *schema.ResourceData, client *golangsdk.ServiceClient, code string) error {
	instanceId := d.Get("instance_id").(string)
	opt := applications.AppCodeOpts{
		AppCode: code,
	}
	if _, err := applications.CreateAppCode(client, instanceId, d.Id(), opt).Extract(); err != nil {
		return err
	}
	return nil
}

func createApigV2ApplicationCodes(d *schema.ResourceData, client *golangsdk.ServiceClient, codes []interface{}) error {
	for _, v := range codes {
		if err := createApigV2ApplicationCode(d, client, v.(string)); err != nil {
			return fmt.Errorf("Error creating APIG v2 application code: %s", err)
		}
	}
	return nil
}
...
func resourceApigApplicationV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating HuaweiCloud APIG v2 application at the instance (%s): %s", instanceId, err)
	}
	d.SetId(resp.Id)
	if v, ok := d.GetOk("app_codes"); ok {
		if err := createApigV2ApplicationCodes(d, client, v.(*schema.Set).List()); err != nil {
			return fmt.Errorf("Error creating APIG v2 application codes: %s", err)
		}
	}
...
func setApigApplicationCodes(d *schema.ResourceData, config *config.Config, resp *applications.Application) error {
	...
	if err != nil {
		return fmt.Errorf("Error getting APIG v2 application codes: %s", err)
	}
...
func removeApigV2ApplicationCode(d *schema.ResourceData, client *golangsdk.ServiceClient,
	...
	if !ok {
		return fmt.Errorf("Code is not in the application (%s)", d.Id())
	}
	if err := applications.RemoveAppCode(client, instanceId, d.Id(), id).ExtractErr(); err != nil {
		return fmt.Errorf("Error removing code (%s) form the application (%s) : %s", code, d.Id(), err)
	}
	return nil
}
...
func resourceApigApplicationV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if opt != (applications.AppOpts{}) {
		...
		if err != nil {
			return fmt.Errorf("Error updating HuaweiCloud APIG v2 application (%s): %s", d.Id(), err)
		}
	}
	if d.HasChange("app_codes") {
		err = updateApigApplicationCodes(d, client)
		if err != nil {
			return fmt.Errorf("Updating HuaweiCloud APIG v2 application code failed: %s", err)
		}
	}
	if d.HasChange("secret_action") {
		if v, ok := d.GetOk("secret_action"); ok && v.(string) == "RESET" {
			if _, err := applications.ResetAppSecret(client, d.Get("instance_id").(string), d.Id(),
				...
				return fmt.Errorf("Reseting HuaweiCloud APIG v2 application secret failed: %s", err)
			}
		}
	}
	return resourceApigApplicationV2Read(d, meta)
...
func resourceApigApplicationV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error deleting HuaweiCloud APIG v2 application from the instance (%s): %s", instanceId, err)
	}
	d.SetId("")
	return nil
}

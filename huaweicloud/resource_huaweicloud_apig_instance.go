...
func buildMaintainEndTime(maintainStart string) (string, error) {
	...
	if len(result) < 2 {
		return "", fmt.Errorf("Match result length is less than 2")
	}
	num, err := strconv.Atoi(result[1])
	if err != nil {
		return "", fmt.Errorf("The number (%s) cannot be converted to string", result[1])
	}
	if len(strconv.Itoa((num+4)%24)) == 1 {
		return fmt.Sprintf("0%d:00:00", (num+4)%24), nil
	}
	return fmt.Sprintf("%d:00:00", (num+4)%24), nil
}
...
func buildApigInstanceParameters(d *schema.ResourceData, config *config.Config) (instances.CreateOpts, error) {
	opt := instances.CreateOpts{
		...
		EipId:               d.Get("ingress.0.eip_id").(string),
		BandwidthSize:       d.Get("egress.0.bandwidth_size").(int), // Bandwidth 0 means turn off the egress access.
...
func waitForApigInstanceCreateCompleted(d *schema.ResourceData, client *golangsdk.ServiceClient) error {
	ref := Refresh{
		Pending:      []string{"Creating", "Initing", "Installing"},
...
func setApigEgressAccess(d *schema.ResourceData, resp instances.Instance) error {
	if resp.BandwidthSize == 0 {
		return d.Set("egress", nil)
	}
	egress := map[string]interface{}{
		"bandwidth_size": resp.BandwidthSize,
	}
	if resp.Ipv4EgressAddress != "" {
		egress["ipv4_address"] = resp.Ipv4EgressAddress
	}
	result := []map[string]interface{}{egress}
	return d.Set("egress", result)
}
...
func setApigIngressAccess(d *schema.ResourceData, config *config.Config, resp instances.Instance) error {
	...
	if resp.Ipv4IngressEipAddress != "" {
		...
		ingress["ipv4_address"] = publicAddress
		...
		if len(publicIps) == 0 {
			return fmt.Errorf("Error getting public ip (%s) from server: %s", publicAddress, err)
		}
		ingress["eip_id"] = publicIps[0].ID
		result[0] = ingress
		return d.Set("ingress", result)
	}
	return d.Set("ingress", nil)
}
...
func enableApigInstanceEgressAccess(client *golangsdk.ServiceClient, id string, size int) error {
	opts := instances.EgressAccessOpts{
		BandwidthSize: strconv.Itoa(size),
	}
	egress, err := instances.EnableEgressAccess(client, id, opts).Extract()
	if err != nil {
		return fmt.Errorf("Unable to enable egress bandwidth of the dedicated instance (%s), size: %d", id, size)
	}
...
func disableApigInstanceEgressAccess(client *golangsdk.ServiceClient, id string) error {
	err := instances.DisableEgressAccess(client, id).ExtractErr()
	if err != nil {
		return fmt.Errorf("Unable to disable egress bandwidth of the dedicated instance (%s)", id)
	}
	return nil
}

func updateApigInstanceEgressAccess(d *schema.ResourceData, client *golangsdk.ServiceClient) error {
	oldVal, newVal := d.GetChange("egress")
	// Enable the egress access.
	if len(oldVal.([]interface{})) == 0 {
		return enableApigInstanceEgressAccess(client, d.Id(), d.Get("egress.0.bandwidth_size").(int))
	}
	// Disable the egress access.
	if len(newVal.([]interface{})) == 0 {
		return disableApigInstanceEgressAccess(client, d.Id())
	}
	// Update the egress nat.
	size := d.Get("egress.0.bandwidth_size").(int)
	opts := instances.EgressAccessOpts{
		BandwidthSize: strconv.Itoa(size),
	}
...
func updateApigInstanceIngressAccess(d *schema.ResourceData, client *golangsdk.ServiceClient) (err error) {
	oldVal, newVal := d.GetChange("ingress")
	...
	if len(oldVal.([]interface{})) != 0 {
		...
		if err != nil || len(newVal.([]interface{})) == 0 {
			return
		}
	}
...
func resourceApigInstanceV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("egress") {
		if err = updateApigInstanceEgressAccess(d, client); err != nil {
			return fmt.Errorf("Unable to update egress access of the APIG v2 dedicated instance: %s", err)
		}
	}
	// Update ingerss access
	if d.HasChange("ingress") {
		if err = updateApigInstanceIngressAccess(d, client); err != nil {
			return fmt.Errorf("Unable to update ingress access of the APIG v2 dedicated instance: %s", err)
		}
	}

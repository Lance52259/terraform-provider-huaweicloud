...
* `algorithm` - (Optional, String) Specifies the type of the backend service, default to WRR.
  The valid types are as following:
  - WRR (default)
  - WLC
  - SH
  - URI hashing

* `protocol` - (Optional, String) Specifies the protocol for performing health checks on backend servers in the VPC channel..

...
Manages an VPC channel resource within HuaweiCloud.
...
* `members` - (Optional, List) Specifies an array of one or more backend server IDs or IP address that bind the VPC
  channel.
...
VPC Channels can be imported using their `id` and ID of the APIG dedicated instance to which the application

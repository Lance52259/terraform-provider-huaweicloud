...
Manages a APIG dedicated instance resource within Huaweicloud.
...
* `egress` - (Optional, List) Specifies the egress access of the APIG dedicated instance.
  The `egress` object structure is documented below.

* `ingress` - (Optional, List) Specifies the ingress access of the APIG dedicated instance.
  The `ingress` object structure is documented below.
...
* `create_time` - Time when the APIG instance is created. The time is in the Unix timestamp format.
...
* `egress/ipv4_address` - The egress public ip address.
* `ingress/ipv4_address` - The ingress eip address.
...
APIG Dedicated Instances can be imported by their `id`. For example,
```

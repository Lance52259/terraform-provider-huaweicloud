...
* `custom_responses` - (Optional, List) Specifies the list of one or more custom responses about the API group.
  The custom_responses structure is documented below.

* `environments` - (Optional, List) Specifies the list of one or more environments about the API group.

The `custom_responses` block supports:

  * `response_id` - (Required, String) Specifies the ID of the custom resource for API group.

  * `name` - (Required, String) Specifies the ID of the resource to be backed up.

  * `responses` - (Required, List) Specifies the ID of the resource to be backed up.
    The responses structure is documented below.

The `responses` block supports:

The `environments` block supports:

  * `environment_id` - (Required, String) Specifies the ID of the custom resource for API group.

  * `variables` - (Required, List) Specifies the ID of the resource to be backed up.
    The responses structure is documented below.
